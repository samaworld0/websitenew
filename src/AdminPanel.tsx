...
import { invitations as defaultInvitations } from "./data"
import {
  supabase,
  isAdminLoggedIn,
  loadInvitations,
...
export function AdminPanel({ onClose }: { onClose: () => void }) {
  ...
  // جدول Supabase ينزرع تلقائياً بالقوالب الافتراضية أول مرة فقط (لما يكون
  // فاضي بالكامل) — فأي قالب جديد يُضاف لاحقاً بملف data.ts (مثل قوالب
  // تصاميم جديدة) ما يوصل تلقائياً للموقع المباشر لأن الجدول مو فاضي.
  // هالزر يقارن القوالب الموجودة بالكود مع اللي موجود فعلياً بقاعدة البيانات
  // ويضيف بس الناقص (بدون ما يلمس أو يكرر أي دعوة موجودة أصلاً).
  const missingDefaults = defaultInvitations.filter(
    (d) => !list.some((l) => l.id === d.id),
  )

  const handleSyncDefaults = async () => {
    if (missingDefaults.length === 0) {
      flash("كل القوالب الافتراضية موجودة أصلاً بالموقع ✅")
      return
    }
    const updated = [...list, ...missingDefaults]
    await persistInvitations(updated)
    setList(updated)
    flash(
      `تمت إضافة ${missingDefaults.length} قالب جديد: ${missingDefaults
        .map((d) => d.subtitle)
        .join("، ")} ✅`,
    )
  }
          ...
          <div className="flex items-center gap-2">
            {missingDefaults.length > 0 && (
              <button
                ...
                className="px-4 py-2 rounded-full text-sm font-bold bg-emerald-600 text-white"
                title={missingDefaults.map((d) => d.subtitle).join("، ")}
              >
                + إضافة {missingDefaults.length} قالب جديد بالكود
              </button>
            )}
            <button
              onClick={() => {
