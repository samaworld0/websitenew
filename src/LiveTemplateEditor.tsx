...
import { EditModeProvider, DeselectSurface } from "./LiveEditing"

// محرر التصميم المباشر — يفتح المعاينة الحقيقية للدعوة (نفس المكوّن اللي
// يشوفه الضيف) بس بوضع "تعديل" مفعّل، حتى يضغط الأدمن على أي اسم أو نص
// ويسحب مقبضه تحته لتكبيره/تصغيره بحرية، بنفس فكرة فيغما.
export function LiveTemplateEditor({
  ...
  const sizesRef = useRef<Record<string, number>>(inv.textSizes || {})
  ...
  const handleSave = () => {
    onSave({ ...inv, textSizes: sizesRef.current })
    setSaved(true)
  ...
  return (
    <div className="fixed inset-0 z-[500] flex flex-col w-full h-full bg-[#0D0706]">
      ...
      >
        ✏️ اضغط على أي نص بالدعوة، وبعدها اسحب علامة ⇕ تحته لتكبيره أو
        تصغيره (أو استخدم + / −)
      </div>
