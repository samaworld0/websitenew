...
export function AdminEditForm({
  ...
  // دالة قراءة الملف المرفوع وتحويله إلى Base64
  const handleFileUpload = (key: keyof EditableInvitation, file: File | null) => {
    if (!file) return
    const reader = new FileReader()
    reader.onload = (e) => {
      if (e.target && e.target.result) {
        updateField(key, e.target.result as string)
      }
    }
  ...
  return (
    ...
    >
      ...
      <div className="md:col-span-2">
        ...
        <div className="flex gap-2">
          <input
            ...
            dir="ltr"
          />
          <label className="shrink-0 px-6 py-2.5 bg-[#B8862F] hover:bg-[#9E7024] text-white rounded-xl font-bold cursor-pointer transition flex items-center gap-2">
            <span>📎 رفع صورة</span>
            <input
              type="file"
              accept="image/*"
      ...
      <div className="md:col-span-2">
        ...
        <p className="text-[11px] text-muted-foreground mt-2 leading-relaxed">
          ملاحظة: رفع مقاطع الفيديو كـ Base64 قد يسبب مشاكل في الحفظ مع قواعد البيانات (Supabase). يُنصح بضغط الفيديو وتقليل حجمه لأقصى حد.
        </p>
      </div>
